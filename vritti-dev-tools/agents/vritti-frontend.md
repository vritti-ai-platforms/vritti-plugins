...
- Surfaces from `@vritti/quantum-ui/PermissionGate`:
  ...
  - `permission` on a `Tabs` `TabItem` — gate a single **tab**: not granted → dropped; granted but locked → visible, disabled, lock + upsell tip; default selection skips locked/disabled tabs.
